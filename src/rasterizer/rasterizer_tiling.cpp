...
void rasterizer_tiling_cfg::set_cluster_strips(rasterizer_tile_cluster_strip *strips_, rasterizer_cluster_strip_index_t max_cluster_strips_)
{
  cluster_strips=strips_;
...
void rasterizer_tiling::init(const rasterizer_tiling_cfg &cfg_)
{
  ...
  PFC_ASSERT(cfg_.tiles && cfg_.clusters && cfg_.cluster_strips);
  PFC_ASSERT_MSG((cfg_.tile_width&(cfg_.tile_width-1))==0 && (cfg_.tile_height&(cfg_.tile_height-1))==0, ("Tile dimensions (%i x %i) must be power-of-2\r\n", cfg_.tile_width, cfg_.tile_height));
  m_cfg=cfg_;
  ...
  PFC_ASSERT_MSG(tiled_width<256 && tiled_height<256, ("Too small tile size (%i x %i) for resolution %i x %i\r\n", cfg_.tile_width, cfg_.tile_height, cfg_.rt_width, cfg_.rt_height));
  ...
  do
  {
    tile->tile_min.x=cfg_.tile_width;
    tile->tile_min.y=cfg_.tile_height;
    tile->tile_end.x=0;
    tile->tile_end.y=0;
...
{
  add_cluster(vec2f(0.0f, 0.0f), vec2f(1.0f, 1.0f), 0.0f, dispatch_idx_, local_cluster_idx_);
}
//----
...
void rasterizer_tiling::add_cluster(const mat44f &v2p_, const vec3f &pos_, ufloat_t rad_, rasterizer_dispatch_index_t dispatch_idx_, rasterizer_local_cluster_index_t local_cluster_idx_)
{
  ...
  float min_z=pos_.z-rad_, q=min_z*v2p_.z.z+v2p_.w.z;
  add_cluster(vec2f(0.5f+0.5f*bbox.x, 0.5f-0.5f*bbox.w), vec2f(0.5f+0.5f*bbox.z, 0.5f-0.5f*bbox.y), q>0.0f?q/min_z:0.0f, dispatch_idx_, local_cluster_idx_);
}
//----

void rasterizer_tiling::add_cluster(const vec2f &bbox_min_, const vec2f &bbox_max_, float min_z_, rasterizer_dispatch_index_t dispatch_idx_, rasterizer_local_cluster_index_t local_cluster_idx_)
{
  // calculate cluster tile bounds
  PFC_ASSERT(m_cfg.tiles);
  PFC_ASSERT_MSG(local_cluster_idx_<rasterizer_max_local_clusters, ("Local cluster index (%i) must be smaller than %i\r\n", local_cluster_idx_, rasterizer_max_local_clusters));
  uint16_t bbox_min_x=(uint16_t)max<int16_t>(0, int16_t(bbox_min_.x*m_cfg.rt_width));
  uint16_t bbox_end_x=(uint16_t)min<int16_t>(m_cfg.rt_width, int16_t(bbox_max_.x*m_cfg.rt_width)+1);
  uint16_t bbox_min_y=(uint16_t)max<int16_t>(0, int16_t(bbox_min_.y*m_cfg.rt_height));
  uint16_t bbox_end_y=(uint16_t)min<int16_t>(m_cfg.rt_height, int16_t(bbox_max_.y*m_cfg.rt_height)+1);
  ...
  cluster.hiz_x=uint8_t(bbox_min_x/rasterizer_hiz_tile_size);
  cluster.hiz_width=uint8_t((bbox_end_x+rasterizer_hiz_tile_size-1)/rasterizer_hiz_tile_size-cluster.hiz_x);
  cluster.hiz_y=uint8_t(bbox_min_y/rasterizer_hiz_tile_size);
  cluster.hiz_height=uint8_t((bbox_end_y+rasterizer_hiz_tile_size-1)/rasterizer_hiz_tile_size-cluster.hiz_y);
  cluster.min_z=uint16_t(65535.0f*min_z_);
#endif
  ...
  do
  {
    ...
    do
    {
      rasterizer_tile &tile=m_cfg.tiles[m_cfg.tile_map?*tmap:uint16_t((usize_t(tmap)/sizeof(*tmap)))];
      rasterizer_tile_cluster_strip *cstrip;
      unsigned cis_offs=(tile.num_clusters++)%rasterizer_max_strip_clusters;
      if(!cis_offs)
      {
        // allocate cluster strip for the tile
        PFC_ASSERT_MSG(m_num_cluster_strips<m_cfg.max_cluster_strips, ("Out of cluster strips (%i)\r\n", m_cfg.max_cluster_strips));
        rasterizer_cluster_strip_index_t cstrip_idx=m_num_cluster_strips++;
        cstrip=m_cfg.cluster_strips+cstrip_idx;
        ...
        {
          (m_cfg.cluster_strips+tile.cluster_strip_last)->next=cstrip_idx;
          tile.cluster_strip_last=cstrip_idx;
        }
      }
      else
        cstrip=m_cfg.cluster_strips+tile.cluster_strip_last;
